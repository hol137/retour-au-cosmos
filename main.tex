...
\subsection{Le carcan du réductionnisme moderne}
    ...
    Il est significatif que ce terme d'immergence soit un parfait néologisme. Autrement dit, le concept de Cosmos a été éradiqué de la pensée commune depuis longtemps. On peut attribuer l'origine de cette déviation au siècle des soi-disant "lumières" où Descartes, en particulier, cherchait à expliquer le mouvement des astres par des tourbillons d'éther faisant avancer les planètes dans le même sens. Hélas pour lui, on s'aperçut que certains objets celestes tels que les comètes avaient un mouvement rétrograde.
...
L'holographie est la maitrise de l'information véhiculée par les ondes. Or comme tout se propage par onde,\textit {l'holographie est centrale dans la Science}. 
...
C'est par exemple le principe des émissions radio à modulation de fréquence. Ainsi le récepteur radio doit disposer d'un oscillateur interne pour reproduire la fréquence qui permet d'extraire le signal utile. Dans ce cas l milieux holographique est l'espace, et l'iformation est déployée dans le temps.
...
Cette idée simpliste et apparue plusieurs fois. Ainsi Mieczysław Wolfke l'a suggéré en 1920 pour les fasceaux X en cristallographie, mais ne l'a pas concrété \cite{Gabor}. Ensuite Dennis Gabor, désirant l'appliquer pour les ondes de la microscopie électronique, l'a concrétisé en optique en 1947. Ce fut une surprise totale pour le opticiens. Ceux-ci n'avaient pas songé à cette technique, car ils disposent d'un moyen direct pour traiter l'informatio optique: ce sont les instruments classiqus, la lunette, le microscope.



L'histoire raconte ue l'idée a germée dans l'esprit de Dennis Gabor, alors qu'il était assis dans un parc. Confronté au problème délicat du microscope électronique, où les lentilles électroniques sont de très mauvaise qualité, il s'est posé a question : peut-on faire de l'imagerie sans lentilles ? Poser la question appelait la réponse immédiate : il suffit de disposer d'une onde cohérente, comme en radiophonie. Cela anticipait le laser, voir ci-dessous.


Gabor a ajouté un concept additif : si on agrandit le récepteur d'un facteur $F$, et si l'on le démodule avec un faisceau de restitution ayant une géomtrie ( y comprisla longuer d'onde) du même facteur $F$, on obtient une testitution parfaite agrandie du même facteur. Ce 'principe de Gabor' allait être très utile dans la suite, et il a conduit récemment à la formulation théorique donnant le masses du photon et du graviton \cite{Sanchez}.
...
L'idée a été retrouvée vers 1970 par Iouri Denisyuk, en utilisant une porteuse allant en sens contraire du signal. Les effets d'épaisseur dans l'émulsion photographique ont éliminé l'image parasite, mais il n'a pu matérialiser cette technique que pour enregistrer une scène contenant une seule information : il holographia un simple miroir convexe, fabricant ainsi un autre type d' "élément optique holographique", nettement supérieur à celui de Roger.
...
Puis vint le tour des ondes radar. Car les militaires avaient beaucoup investi dans ce domaine, et, dans le laboratoire du Michigan, Emmet Leith voulut utiliser l'idée de Gabor, en se ramenant à l'optique, mais en séparant spatialemnt la porteuse de l'onde-signal, il résolvait le problème de l'image parasite. Il obtint ainsi, avec son collègue estonien Upatnieks, qui n'avait pas le droit de s'occupper des questions radar, ultra-secrètes, des images de transparents 2D de bone qualité. L'étape suivant était de passer à l'holographie d'objets 3D. Mais la lumière diffusée était trop faible. Par un curux concours de circonstante, c'est à ce miment précis que le labo vit arive e premier laser, à Néon/Hélium. Bien que de fabe puissance nominale (milliwat), le gain en luminosité cohérente était énorme, et près un an por régler le problème des franfes parasits que donnait une telle cohérence, Leith et Upatnieks obtinrent les premières images holographiques.


Le succès fut immédiat, à tel point que le nouveau directeur du labo, fraichement débarqué depuis l'Europe, George Stroke voulut s'approprier une telle avancée, conformément à la ouitume des llabos européens. Surtout que E. Leith n'avait pas encre sa thèse. Stroke écrivit raidement le premier ouvrage dur l'holographie, en faussant la réalité historique et milita pour que le prix Nobel soit octroyé à Gabor seul, avec éventuellement lui-même. Gabor reçut le prix en 1971, mais, dans son discours, il minimisa l'apport décisif de Leith et Upatnieks.


Dans l'intervalle trois labos américains redécouvrirent par hasard la méthode à porteuse arrière de Denisyuk, qui du coup fut finalement reconnu en Russie. Quelques années plus tard, Debnisyuk put enfin accéder au laser, il put alors bénéficier pleinement des améliorations décisives qu'il avait efectué avec Rebekka Protas à l'Institut dOtique Vavilov de Leningrad pour réaliser des émusions super-sensibiliées à grains très fins (02 microns). 


C'est ainsi que l'école russe produisit des hologrammes qui battirent à plate couture les hologrammes américains et européens. En effet il suffisait d'un simple spot pour restituer l'image alors que les autres nécessitaient une lampe spectrale puissante, voir m\^eme un laser pour les plus grands modèles.



C'est alors que l'école française reprit la main, en éliminant totalement la nécessité d'un lourd banc, par la méthode de l'hologramme contact, et la possibilté du balayage permettant de faire de grands hologrammes avec de faibles faisceaux lasers, ce qui conduisit à la réalisation d'un metre caqrré avec un fasceau milixwatt (Juillet 1987). Par la suite Yves Gentet utilisa ces techniques pour obtenir les hologrammes-couleurs, les plus bees images, qui viennent compléter les célèbres images 2D en vraies couleurs de Gabriel Lippmann. 



A noter que Denisyuk s'était inspiré de ces travaux préccurseurs, qu'on peut qu'on ne peut qualifier d'holographie, car seule la vraie couleur est enregistée localement, grace au bain de mercure sevant de miroir en contact avec l'émuleion photograohique à grains ultra-fins.


Mais à cause de ces décalages temporls, la littérature officielle ne fit pads grad cas des hologrammes restituables en lumière blanche, et les textes prése,taiernt le motge de Leith comme indispensable. Ceci à tel point que, avec le laser Argon plus puissant, plusieurs labos ne parvenrent pas à faire le moindre hologrammez: à cause du refroidissement qui faisait vibrer. Aucune mention n'était faue sur ce point essentiel : la rigidité du montage n'es nécassire qu'après séparation des faisceaux, c'est pourquoi il est indispensale d'indtaller le laser sur une table séparée.


Ainsi plusieurs thèses discordantes furent développées dans les diférents ouvrages, sans parler de l'embalement médiatique qui suivit. Même Gabor ne put s'empécher de faire le rapprochement entre l'hogramme et la mémoire humaine, en remarquant que chaque point de l'hologramme contient de l'information sur l'ensemble de la scène, de même que la mémoire n'est pas locaisée dans une zone particulière du cerveau.


Plus fondamentalement, du fait de l'universalité de l'holographie il fallait inreoduire un Principe Holographique en Physique Théorique, voir plus haut.
